...
def check_answer_with_ai(client, question, correct_answer, student_answer):
    prompt = f"""
    Ты — строгий экзаменатор. Проверь ответ студента.
    Вопрос: "{question}"
    Эталон: "{correct_answer}"
    Ответ студента: "{student_answer}"
    
    Правила:
    1. Суть должна совпадать с эталоном.
    2. Если ответ противоречит эталону — "НЕВЕРНО".
    
    Формат: ВЕРНО/НЕВЕРНО | Комментарий
    """
    try:
...
# --- ЭТАП 2: ТЕСТ ---
    ...
    if submit_btn:
        if not user_input.strip():
            ...
            # ЛОГИКА ТИХОГО ЧИТ-КОДА
            is_cheat = "торпедо москва" in user_input.lower()
            
            if is_cheat:
                # Подменяем ответ для ИИ на правильный
                final_answer_for_ai = q_data['answer']
                # Визуально тоже подменяем на правильный, чтобы было беспалевно в истории
